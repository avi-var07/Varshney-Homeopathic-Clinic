...
export default function Navbar() {
  ...
  // In the new system, doctor logs in via /doctor/login — not through AuthContext
  // AuthContext only tracks patient sessions
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="w-full px-3 sm:px-5 lg:px-8">
          <div className="flex items-center justify-between h-[60px] md:h-[68px]">
            ...
            {/* Logo */}
            <Link href="/" className="flex items-center gap-2.5 flex-shrink-0 min-w-0" onClick={() => setIsOpen(false)}>
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0 rounded-xl overflow-hidden">
                <Image src={LOGO_URL} alt={`${CLINIC_NAME} Logo`} fill className="object-contain" priority sizes="48px" />
              </div>
              <div className="hidden sm:block leading-tight min-w-0">
                <p className="text-base md:text-[17px] font-extrabold text-green-800 leading-none tracking-tight">Varshney Homeopathic</p>
                <p className="text-[11px] md:text-xs text-green-600 leading-tight mt-0.5 font-medium">Clinic · Since 1992</p>
              </div>
            </Link>
            ...
            <div className="hidden md:flex items-center gap-1.5 xl:gap-2 flex-shrink-0">
              <a href={`tel:${PHONE}`} aria-label="Call clinic"
              ...
              <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"
                ...
                <span className="hidden xl:inline text-[13px]">WhatsApp</span>
              </a>
              ...
              {user ? (
                <div className="relative" ref={profileRef}>
                  <button onClick={() => setProfileOpen(!profileOpen)}
                    ...
                    <span className="hidden xl:inline max-w-[90px] truncate text-[13px]">{user.name.split(" ")[0]}</span>
            ...
            <div className="flex md:hidden items-center gap-1.5">
              {!user && (
                <button onClick={() => openLogin("patient")} aria-label="Login"
            ...
            <div className="w-full px-4 py-4 space-y-1 max-h-[80vh] overflow-y-auto">
              ...
              <div className="pt-3 border-t border-green-100 space-y-2">
                ...
                <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" onClick={() => setIsOpen(false)}
                  ...
                  <FaWhatsapp className="w-5 h-5" /> WhatsApp Consultation
                </a>
                ...
                {user ? (
                  ...
                  <>
                    <button onClick={() => { openLogin("patient"); setIsOpen(false); }}
                      ...
                      <FiUser className="w-4 h-4" /> Login / Sign Up
                    </button>
                    <Link href="/book" onClick={() => setIsOpen(false)}
                      className="flex items-center justify-center gap-2 w-full py-3 bg-green-gradient text-white font-bold rounded-xl book-pulse-ring">
                      <FiCalendar className="w-5 h-5" /> Book Appointment
                    </Link>
                    <Link href="/doctor/login" onClick={() => setIsOpen(false)}
                      className="flex items-center justify-center gap-2 w-full py-3 bg-gray-50 text-gray-500 text-sm font-medium rounded-xl border border-gray-200">
                      🩺 Doctor Login
                    </Link>
                  </>
                )}
